  ...
  return (
    <div className="py-8">
      <section className="mb-12 bg-gradient-to-br from-indigo-900 to-indigo-800 rounded-[2.5rem] p-8 md:p-20 text-white shadow-2xl text-center relative overflow-hidden">
        <div className="relative z-10">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6">
            <button onClick={() => scrollToSection('trending-section')} className="bg-white text-indigo-900 px-12 py-5 rounded-2xl font-black text-xl hover:bg-indigo-50 transition-all">اكتشف الترند 🔥</button>
            <button onClick={() => scrollToSection('latest-section')} className="bg-indigo-700/40 text-white border-2 border-white/20 px-12 py-5 rounded-2xl font-black text-xl hover:bg-white/10 transition-all">أحدث الإضافات ✨</button>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white rounded-[3rem] p-12 shadow-sm border border-gray-100 text-center">
        <h3 className="text-2xl font-black mb-10">تصفح حسب التصنيفات</h3>
        <div className="flex flex-wrap justify-center gap-4">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => handleCategoryClick(cat)} className="bg-gray-50 px-10 py-4 rounded-2xl font-bold text-gray-700 hover:border-indigo-600 border-2 border-transparent transition-all">
              {cat}
            </button>
          ))}
        </div>
